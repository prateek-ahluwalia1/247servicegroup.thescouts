...
class Announcement extends Model
{
    ...
    public function inductions(){
        $results=DB::table('tutorials')->select('id','title','html_body','TIMESTAMP_LAST_UPDATED as updated_at', 'send_to', 'send_by', 'send_by_list', 'selected_guards')->where('status','!=','deleted')->get();
        return $results;
    ...
    public function get_guard_list($send_by,$send_by_list_id){
        if ($send_by == 'customer') {
            ...
            // ->where('jobs.customer_id', '=', $send_by_list_id)
            // ->select('guards.*')
            // ->groupBy('guards.id')
          ...
          elseif($send_by == 'contractor'){
            $results=DB::table('guards')->where('contractor_id', $send_by_list_id)->orderBy('name','ASC')->get();
          }else{
            // ->where('guard_type','Contractor')
              $results=DB::table('guards')
              ->where('guards.status', 'active')
             ->where('guards.is_approved', 'yes')
             ->where('guards.admin_approved', 1)
        ...
        public function add_induction($title,$send_to,$send_by,$htmlBody,$send_by_list,$send_to_guards){
            $results=DB::table('tutorials')->insert([
                'title' => $title,
                'send_to'=>$send_to,
                'send_by'=>$send_by,
                'html_Body'=>$htmlBody,
                ...
                'TIMESTAMP_INSERTED' => '',
                'selected_guards' => $send_to_guards,
                'status' => 'active'
            ]
            );
            return $results;
        ...
        public function edit_induction($title,$send_to,$send_by,$htmlBody,$send_by_list,$send_to_guards,$inductionId){
            $results=DB::table('tutorials')->where('id',$inductionId)->update([
                'title' => $title,
                'send_to'=>$send_to,
                'send_by'=>$send_by,
                'html_Body'=>$htmlBody,
