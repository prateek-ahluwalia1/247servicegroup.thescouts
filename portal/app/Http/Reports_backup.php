...
function generate_incident_report_pdf_new(Request $request, $id)
  {
    ...
    ->where('incident_reports.id', $id)->select('incident_reports.*', 'guards.name as guard_name', 'guards.phone', 'jobs.address', 'jobs.site_name', 'jobs.site_description', 'job_new_roster.job_start', 'job_new_roster.job_end')->first();
    ...
    <div style="text-align: center;" class="title">'.strtoupper(config('custom.title')).' SECURITY INCIDENT REPORT</div>
    </div>
    </td>
    </tr>
    </table>
    ...
    <td>
    <div>
    <strong>Date of Incident:</strong> '.date('d-m-Y', strtotime($report->incident_date)).'
    </div>
    </td>
    <td>
    <div>
    <strong>Time of Incident:</strong> '.date('H:i:s', strtotime($report->incident_time)).'
    </div>
    </td>
    </tr>
    </table>';
    ...
    <td>
    <div>
    <strong>Location of Incident:</strong> '.$report->site_name.'
    </div>
    </td>
    <td>
    <div>
...
// print_r('<pre>');
  ...
  {
    print_r('<pre>');
    print_r($this->getShiftHours(date('m/d/Y H:i', strtotime('2023-04-25 17:45:00')), date('m/d/Y H:i', strtotime('2023-04-26 06:45:00'))));
  }
  ...
  j.`apply_date`,
        ...
        foreach ($results as $result) {
            ...
            if (strtotime($result->apply_date) <= strtotime($result->temp_start)) {
              ...
              ->orderBy('apply_date', 'desc')->first();
              if (!empty($old_payrate)) {
                $guard_rates = $payrates = DB::table('payrates')->where('id', $old_payrate->payrate_id)->where('archive', 0)->first();
              }
            }
          } else {
            $guard_rates = $payrates = DB::table('payrates')->where('archive', 0)->where('id', $result->site_payrate)->first();
                ...
                if(config('custom.own_payrates') == 1 && $result->own_payrate == 'custom')
      {
        ...
        if ($result->own_payrate == 'award' && $result->pay_by != 'award') {
            // echo "i am here1";
        ...
        }elseif (config('custom.own_payrates') == 1 && $result->pay_by == 'award' && strtotime($result->award_rate_apply_from) <= strtotime($result->temp_start) && strtotime($result->award_rate_apply_to) > strtotime($result->temp_start)) {
            $guard_rates = $payrates = DB::table('award_payrates')->where('level', $result->award_rate_level)->first();
            if (!empty($guard_rates)) {
                $payrates->ot_base_rate = 22.84;
            }else{
                $guard_rates = $payrates = array();
      ...
      }elseif(config('custom.own_payrates') == 1 && $result->pay_by == 'award' && strtotime($result->award_rate_apply_from) <= strtotime($result->temp_start) && strtotime($result->award_rate_apply_to) > strtotime($result->temp_start))
      {
        ...
        $sunday_day_rate = $payrates->casual_sat;
        $sunday_night_rate = $payrates->casual_sat;
        $saturday_day_rate = $payrates->casual_sun;
        $saturday_night_rate = $payrates->casual_sun;
        ...
        $sunday_day_rate = $payrates->pf_sat;
        $sunday_night_rate = $payrates->pf_sat;
        $saturday_day_rate = $payrates->pf_sun;
        $saturday_night_rate = $payrates->pf_sun;
...
if (isset($request['state']) && $request['state'] != '') {
    ...
    $extra_query .= ") AND ";
        ...
        foreach ($results as $result) {
            ...
            if ($result->shift_chargerate_id > 0) {
