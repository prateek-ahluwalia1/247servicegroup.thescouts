...
use App\Models\Induction as induction;

class Inductions extends Controller
{
  public $guard_model;
  public $induction_model;
  public function __construct(Request $request, induction $induction_model, guard $guard_model) {
  ...
  public function index($call_from = null){
    ...
    if ($call_from == 'api') {
      ...
      return view('admin/induction/index',['results'=>$results]);
    }
  }   
  public function induction($id){
    $results=DB::table('inductions')->select('id','title','html_body','TIMESTAMP_LAST_UPDATED as updated_at')->where('id',$id)->get();
    foreach($results as $r){
      $r->updated_at=date('d-m-y',strtotime($r->updated_at));
    }
    return view('admin/induction/induction',['results'=>$results]);
  }   
  ...
  public function add_induction(Request $request){
    $results= $this->induction_model->add_induction($request->title,$request->send_to,$request->send_by,$request->htmlBody,$request->send_by_list,$request->send_to_guards);
    if($results)
    {
 ...
 public function edit_induction(Request $request){
  $results= $this->induction_model->edit_induction($request->title,$request->send_to_edit,$request->send_by_edit,$request->htmlBody_edit,$request->send_by_list_edit,$request->send_to_guards_edit,$request->inductionId);
  if($results)
  {
...
public function update_induction(Request $request){
  $results =DB::table('inductions')->where('id',$request->id)->get();
...
public function delete_induction( Request $request){
  $results=DB::table('inductions')->where('id',$request->id)->update([
    'status' => 'deleted',
  ]
);
  if($results){
...
function get_induction_images_api(Request $request){
  return $this->get_induction_images($request, 'api');
}


function get_induction_images(Request $request, $call_from = null)

{
  ...
  if ($call_from != null && $call_from == 'api') {
    if (count($results) > 0) {
            return response()->json(['status' => true, 'message' => 'Data found', 'data' =>  $results]);
        }else{
            return response()->json(['status' => false, 'message' => 'No data found!']);
